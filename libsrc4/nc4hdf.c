...
rec_reattach_scales(NC_GRP_INFO_T *grp, int dimid, hid_t dimscaleid)
{
  ...
  for(iter=0;NC_listmap_next(&grp->children,iter,(void**)&child_grp);iter++) {
  ...
  if (var->became_coord_var)
    {
      ...
      for(iter=0;NC_listmap_next(&grp->dim,iter,(void**)&d1);iter++) {
  ...
  if (dim->extended)
    {
       ...
       * attribute. */
      /* Find var with same name as dim, if any */
      v1 = NC_listmap_get(&grp->vars.value,dim->name);
      if (v1)
        {
      ...
      if (!var->dimscale)
        {
              ...
              for (d = 0; d < var->ndims; d++)
                {
                  ...
                  for (g = grp; g && !finished; g = g->parent)
                    {
		      size_t diter;
		      for(diter=0;NC_listmap_next(&g->dim,diter,(void**)&dim);diter++)
                        {
		  ...
		  for(diter=0;NC_listmap_next(&grp->dim,diter,(void**)&dim);diter++) {
    ...
    for(i = 0; i < nobj; i++) {
		...
		*countp = *countp + 1;
		    ...
		    ra = NC_findreserved(name);
	            if(ra != NULL && (ra->flags & DIMSCALEFLAG))
			*countp = *countp+1;
                }
                H5Aclose(aid);
            }
